...
function Edituser() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            navigate("/profile")
            try {
                const data = await axios.get("https://634579c9dcae733e8ff3a795.mockapi.io/internship", values);
                const data_value=data.filter(x => x.id=="8")
    ...
    return (
        <div className='container'>
            ...
            <Link to={"/profile"} href="#" class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm mb-5"><i
